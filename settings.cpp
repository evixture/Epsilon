...
Settings::Settings(int screenCharWidth, int screenCharHeight)
	:screenCharWidth(screenCharWidth), screenCharHeight(screenCharHeight), windowTitle("Epsilon v. Alpha 7"), fullscreen(false), maxFps(0), fovtype(FOV_RESTRICTIVE), renderer(TCOD_RENDERER_OPENGL2), //USE SDL2/ opengl2 TO RENDER, OTHERS WILL LOCK OR LOWER FPS
...
{
	TCODConsole::root->printf(10, 0, "FPS>%i | mx>%i my>%i | px>%i py>%i pl>%i | winfoc>%i | lft>%f",
		fpsCount,
