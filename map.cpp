	...
	if (testName == "creatures")
	{
		for (auto& creature : dataNode.children()) //for nodes in the creatures 
		{
			...
			else
			{
				...
				Creature creature = Creature(Position4(x, y, level, floor), ch, name, Position3(0, 0, 128), health, Armor("TEMP", armorColor, armorDefense, armorDurability)); //missing armor name in file
